function [signals, microphones, We, Xdis]=bass_deconv(x,L,param)
...
LengthOfICA=param.lengthofICA;
offset=param.offsetICA;
method=param.method;
wlevel=param.weighting;
wtype=param.wtype;
clustype=param.clustering;
mu=param.mu;
useiweights=param.useiweights;
...
%the distance matrix
if param.similarity==1
    R=projections4(comp,L); % projections
elseif param.similarity==2
    R=projections7(comp,L); % coherence
elseif param.similarity==3
    R=projections8(comp,128); % GCC-PHAT
end
...
elseif (strcmp(clustype,'rfcm'))
   [p clst weights_fuzzy xxx xxx Xdis]=bass_rfcm(R,size(x,1),1.2,'wind',1e-4, 0,0);
end

% p... order of clustered signals
R=R(p,p);
